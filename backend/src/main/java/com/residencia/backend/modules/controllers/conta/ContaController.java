...
import com.residencia.backend.modules.services.conta.EditarContaService;
...
public class ContaController {
  ...
  @Autowired
  private EditarContaService editarContaService;
...
@Operation(
  ...
  @Operation(
    ...
    @Operation(
      summary = "Atualizar uma conta recorrente específica",
      method = "PUT",
      description = "Altera os dados de uma conta, passando os novos dados pelo body.",
      responses = {
  ...
  @PutMapping("/{id}")
    public ResponseEntity<Object> update(@PathVariable Integer id, @Valid @RequestBody ContaDTO contaDTO,HttpServletRequest request) {
        try {
            var idUsuario = UUID.fromString(request.getAttribute("id_usuario").toString());
            var resultado = this.editarContaService.execute(id, contaDTO, idUsuario);
            return ResponseEntity.ok().body(resultado);
        } catch (Exception e) {
            return ResponseEntity.badRequest().body(e.getMessage());
        }
